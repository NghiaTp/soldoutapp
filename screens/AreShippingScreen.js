...
const AreShippingScreen = () => {
  const [data, setData] = useState([]);
  const { url, token } = useContext(StoreContext);
  const navigation = useNavigation();

  const fetchOrders = async (status = "Đang vận chuyển") => {
    try {
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.headerText}>Đơn đang vận chuyển</Text>
...
export default AreShippingScreen;

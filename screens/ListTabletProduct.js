...
const ListTabletProduct = ({ navigation }) => {
  const [filteredProducts, setFilteredProducts] = useState([]);
  const [sortOption, setSortOption] = useState("default");
  const [showOnlySale, setShowOnlySale] = useState(false);
  const [selectedBrand, setSelectedBrand] = useState("Tất cả");
  const { url, tabletProduct } = useContext(StoreContext);

  useEffect(() => {
    let result = [...tabletProduct];
  ...
  }, [tabletProduct, sortOption, showOnlySale, selectedBrand]);
  ...
  const brands = [
    "Tất cả",
    ...new Set(tabletProduct.map((product) => product.brand)),
  ];

  return (
    <View style={styles.container}>
      <Text style={styles.title}>Các sản phẩm Tablet</Text>

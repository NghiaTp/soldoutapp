...
const ListPhoneProduct = ({ navigation }) => {
  const [filteredProducts, setFilteredProducts] = useState([]);
  const [sortOption, setSortOption] = useState("default");
  const [showOnlySale, setShowOnlySale] = useState(false);
  const [selectedBrand, setSelectedBrand] = useState("Tất cả");
  const { url, phoneProduct } = useContext(StoreContext);
  ...
  }, [phoneProduct, sortOption, showOnlySale, selectedBrand]);
  ...
  const brands = [
    "Tất cả",
    ...new Set(phoneProduct.map((product) => product.brand)),
  ];

  return (
    <View style={styles.container}>
      <Text style={styles.title}>Các sản phẩm Điện thoại</Text>
